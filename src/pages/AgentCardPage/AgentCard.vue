<template>
    <v-container class="pa-6">
        <v-row>
            <v-col cols="6">
                <v-card class="pa-8" border elevation="0">
                    <v-row>
                        <v-col>
                            <v-card-title>
                                {{ agent.displayName }}
                    ...
                    <div>
                        <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 16px;">
                            <v-chip v-for="(ability, index) in agent.abilities" :key="index"
                                :color="selectedAbilityIndex === index ? 'white' : undefined" variant="outlined"
                                style="cursor: pointer;" @click="selectAbility(index)">
                                {{ ability.displayName }}
                            </v-chip>
                        </div>
                        ...
                        <div v-if="selectedAbility">
                            <v-card elevation="0">
                                <div style="display: flex; align-items: center; gap: 12px;">
                                    <v-img :src="selectedAbility.displayIcon" contain width="40" height="40"
                                        style="flex-shrink: 0;" />
                                    <div>
                                        <div>
            ...
            <v-col cols="4">
                <v-img :src="agent.fullPortrait" :alt="agent.displayName" contain max-height="600" class="pa-4" />
            </v-col>
        </v-row>
    </v-container>
</template>
